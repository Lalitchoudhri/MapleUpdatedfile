codeunit 50018 "Voucher Integration"
{
    ...
    begin
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                if RecResource."Show Json" then
                    Message(GetvoucherdetailsResponse);
    ...
    begin
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                if RecResource."Show Json" then
                    Message(ResponseGetVoucherValue);
                ResponseJsonObject.ReadFrom(ResponseGetVoucherValue);
                ResponseJsonObject.Get('data', ResponseJsonToken);
                dataJsonObject := ResponseJsonToken.AsObject();

                if dataJsonObject.Get('remaining_value', ResponseJsonToken) then
                    remaining_value := ResponseJsonToken.AsValue().AsDecimal();
                //   Message('remaining_value %1', remaining_value);
                if dataJsonObject.Get('expiration_date', ResponseJsonToken) then
                    //  Message(' expiration_date %1', ResponseJsonToken.AsValue().AsText());
    ...
    begin
        ...
        if RecResource."Show Json" then
            Message(Format(MapleAuthJsonObject));
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                if dataJsonObject.Get('authorization_id', ResponseJsonToken) then
                    authorization_id := ResponseJsonToken.AsValue().AsText();
                //   Message('remaining_value %1', remaining_value);
                Clear(authorized_amount);
                if dataJsonObject.Get('authorized_amount', ResponseJsonToken) then
                    //  Message(' expiration_date %1', ResponseJsonToken.AsValue().AsText());
    ...
    begin
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                dataJsonObject := ResponseJsonToken.AsObject();
                Clear(authorization_id);
                if dataJsonObject.Get('refund_id', ResponseJsonToken) then
                    authorization_id := ResponseJsonToken.AsValue().AsText();
                //   Message('remaining_value %1', remaining_value);
                Clear(authorized_amount);
                if dataJsonObject.Get('refunded_amount', ResponseJsonToken) then
                    //  Message(' expiration_date %1', ResponseJsonToken.AsValue().AsText());
                authorized_amount := ResponseJsonToken.AsValue().AsDecimal();
                ResponseJsonObject.Get('meta', ResponseJsonToken);
                metaJsonObject := ResponseJsonToken.AsObject();
    ...
    begin
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                dataJsonObject := ResponseJsonToken.AsObject();
                Clear(authorization_id);
                if dataJsonObject.Get('cancellation_id', ResponseJsonToken) then
                    authorization_id := ResponseJsonToken.AsValue().AsText();
                //   Message('remaining_value %1', remaining_value);
                Clear(authorized_amount);
                if dataJsonObject.Get('cancelled_amount', ResponseJsonToken) then
                    //  Message(' expiration_date %1', ResponseJsonToken.AsValue().AsText());
                authorized_amount := ResponseJsonToken.AsValue().AsDecimal();
                ResponseJsonObject.Get('meta', ResponseJsonToken);
                metaJsonObject := ResponseJsonToken.AsObject();
    ...
    begin
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                ...
                dataJsonObject := ResponseJsonToken.AsObject();
                Clear(CaptureId);
                if dataJsonObject.Get('capture_id', ResponseJsonToken) then
                    CaptureId := ResponseJsonToken.AsValue().AsText();
                Clear(captured_amount);
                if dataJsonObject.Get('captured_amount', ResponseJsonToken) then
                    captured_amount := ResponseJsonToken.AsValue().AsDecimal();
                ResponseJsonObject.Get('meta', ResponseJsonToken);
                metaJsonObject := ResponseJsonToken.AsObject();
                metaJsonObject.Get('status', ResponseJsonToken);
                VoucherStatus := ResponseJsonToken.AsValue().AsText();

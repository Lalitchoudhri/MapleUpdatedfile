codeunit 50041 "Ingram Integration"
{
    ...
    begin
        ...
        IF NOT Location."Ingram Location" THEN
            EXIT;

        IF PurchaseHeader."Ingram Order No." <> '' THEN
            EXIT;
    ...
    begin
        if not JsonObjectValue.SelectToken(path, json_Token) then
    ...
    begin
        ...
        REPEAT
            ...
            PurchLineJsonObject.Add('customerLineNumber', FORMAT(PurchaseLine."Line No."));
        ...
        if HttpClient.Send(HttpRequestMessage, HttpResponseMessage) then
            if HttpResponseMessage.IsSuccessStatusCode() then begin
                if HttpResponseMessage.Content.ReadAs(ResponseText) then
                    OK := JAccessToken.ReadFrom(ResponseText);
                IF OK THEN
    ...
    begin
        ...
        foreach ResponseJsonToken in ordersJsonArray do begin
            ...
            foreach ResponseJsonToken in linesJsonArray do begin
                ...
                IF linesJsonObject.Get('subOrderNumber', ResponseJsonToken) THEN
                    IngramSubOrderNo := ResponseJsonToken.AsValue().AsText();

...
{
    Permissions = tabledata "G/L Entry" = rmdi;
    trigger OnRun()
    begin
    ...
    begin
        ...
        IF SalesInvHeader.FindFirst() THEN BEGIN
            ...
            // LogisiticAPI(SalesInvHeader);
            //LogisiticInvoiceAPI(SalesInvHeader);
        END;

    end;

    procedure LogisiticAPI(SalesInvHeader: Record "Sales Invoice Header");
    var
        HttpClient: HttpClient;
        SecurityHttpClient: HttpClient;
        HttpResponse: HttpResponseMessage;
    ...
    begin
        ...
        SalesInvoiceHeaderObject.Add('slaInHr', 0);//SalesInvHeader."No.");
        SalesInvoiceHeaderObject.Add('paymentStatus', SalesInvHeader."Payment Status");//check
        SalesInvoiceHeaderObject.Add('paymentMode', SalesInvHeader."Payment Status");// SalesInvHeader."Payment Terms Code");
        ...
        SalesInvoiceHeaderObject.Add('shippingAddressLine1', SalesInvHeader."Ship-to Address");
        SalesInvoiceHeaderObject.Add('shippingAddressLine2', SalesInvHeader."Ship-to Address 2");
        SalesInvoiceHeaderObject.Add('shippingAddressCity', SalesInvHeader."Ship-to City");
        SalesInvoiceHeaderObject.Add('shippingAddressState', SalesInvHeader."Ship-to County");
        SalesInvoiceHeaderObject.Add('shippingAddressZipCode', SalesInvHeader."Ship-to Post Code");
        SalesInvoiceHeaderObject.Add('billingCountry', SalesInvHeader."Bill-to Country/Region Code");
        SalesInvoiceHeaderObject.Add('billingAddressLine1', SalesInvHeader."Bill-to Address");
        SalesInvoiceHeaderObject.Add('billingAddressLine2', SalesInvHeader."Bill-to Address 2");
        SalesInvoiceHeaderObject.Add('billingAddressCity', SalesInvHeader."Bill-to City");
        SalesInvoiceHeaderObject.Add('billingAddressState', SalesInvHeader."Bill-to County");
        SalesInvoiceHeaderObject.Add('billingAddressZipCode', SalesInvHeader."Bill-to Post Code");
        SalesInvoiceHeaderObject.Add('marketplace', 'Maple');//Check
        SalesInvoiceHeaderObject.Add('orderDate', FORMAT(SalesInvHeader."Order Date", 0, 9));
        ...
        IF SalesInvLine.FindSet() THEN
            REPEAT
                ...
                SalesInvoiceLineObject.Add('totalAmount', SalesInvLine."Line Amount");
        ...
        SalesInvoiceHeaderObject.Add('invoiceAmount', SalesInvHeader."Amount Including VAT");
        ...
        RequestHeader.Remove('Content-Type');
        RequestHeader.Add('Content-Type', 'application/json');
        AuthString := STRSUBSTNO('Bearer %1', Token());
    ...
    */
    ...
    begin
        with salesHeader do begin
            ...
            i := 1;
            j := 1;
            PendingShieldAmt := "Total Shield Value";
            // 1) cash
            if ("CashPay Method Code" <> '') and (PayByCashAmt <> 0) then begin
                PaymentMethod.Get("CashPay Method Code");
                //  MESSAGE('Cash');
                ...
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    if PendingShieldAmt <> 0 then begin
                        if PayByCashAmt > PendingShieldAmt then begin
                            ArPayTypes[i, j] := Format(PayByCashAmt - PendingShieldAmt);
                            PendingShieldAmt := 0;
                        end
                        else begin
                            ArPayTypes[i, j] := Format(0);
                            PendingShieldAmt := PendingShieldAmt - PayByCashAmt;
                        end;
                    end;
                end
                else
                    ArPayTypes[i, j] := Format(PayByCashAmt);
                j += 1;
                ArPayTypes[i, j] := '';
            ...
            j := 1;
            // 2) cheque
            if ("ChqPay Method Code" <> '') and (PayByChqAmt <> 0) then begin
                ...
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByChqAmt > PendingShieldAmt then begin
                        ArPayTypes[i, j] := Format(PayByChqAmt - PendingShieldAmt);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        PendingShieldAmt := PendingShieldAmt - PayByChqAmt;
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByChqAmt);
                j += 1;
                ArPayTypes[i, j] := '';
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := '0';
            ...
            j := 1;
            // 3) CC1
            if ("CreditCdPay Method Code1" <> '') and (PayByCreditAmt1 <> 0) then begin
                //MESSAGE('Credit1');
                PaymentMethod.Get("CreditCdPay Method Code1");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                j += 1;
                ...
                ServiceCharges := (PayByCreditAmt1 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByCreditAmt1 > PendingShieldAmt then begin
                        ServiceCharges := ((PayByCreditAmt1 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt1 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt1;
                    end;
                    // END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByCreditAmt1 - ((PayByCreditAmt1 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
            ...
            j := 1;
            // 4) CC2
            if ("CreditCdPay Method Code2" <> '') and (PayByCreditAmt2 <> 0) then begin
                PaymentMethod.Get("CreditCdPay Method Code2");
                //MESSAGE('Credit2');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ArPayTypes[i, j] := PaymentMethod."Bal. Account No.";
                j += 1;
                ServiceCharges := (PayByCreditAmt2 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByCreditAmt2 > PendingShieldAmt then begin
                        ServiceCharges := ((PayByCreditAmt2 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt2 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt2;
                    end;
                    /// END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByCreditAmt2 - ((PayByCreditAmt2 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
            ...
            j := 1;
            // 5) CC3
            if ("CreditCdPay Method Code3" <> '') and (PayByCreditAmt3 <> 0) then begin
                PaymentMethod.Get("CreditCdPay Method Code3");
                //MESSAGE('Credit3');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := (PayByCreditAmt3 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByCreditAmt3 > PendingShieldAmt then begin
                        ServiceCharges := ((PayByCreditAmt3 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt3 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - (PayByCreditAmt3);
                    end;
                    // END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByCreditAmt3 - ((PayByCreditAmt3 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
            ...
            j := 1;
            // 6) CC4
            if ("CreditCdPay Method Code4" <> '') and (PayByCreditAmt4 <> 0) then begin
                //MESSAGE('Credit4');
                PaymentMethod.Get("CreditCdPay Method Code4");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                j += 1;
                ...
                ServiceCharges := (PayByCreditAmt4 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if (PayByCreditAmt4 - ServiceCharges) > PendingShieldAmt then begin
                        ServiceCharges := ((PayByCreditAmt4 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt4 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt4;
                    end;
                    // END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByCreditAmt4 - ((PayByCreditAmt4 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
            ...
            j := 1;
            // 7) DC1
            if ("DebitCdPay Method Code2" <> '') and (PayByDebitAmt2 <> 0) then begin
                //MESSAGE('Credit4');
                PaymentMethod.Get("DebitCdPay Method Code2");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                j += 1;
                ...
                ServiceCharges := (PayByDebitAmt2 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByDebitAmt2 > PendingShieldAmt then begin
                        ServiceCharges := ((PayByDebitAmt2 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByDebitAmt2 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - PayByDebitAmt2;
                    end;
                    // END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByDebitAmt2 - ((PayByDebitAmt2 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            j := 1;
            // 8) DC2
            if ("DebitCdPay Method Code1" <> '') and (PayByDebitCdAmt1 <> 0) then begin
                ///MESSAGE('Debit');
                PaymentMethod.Get("DebitCdPay Method Code1");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Bal. Account No.";
                j += 1;
                ServiceCharges := (PayByDebitCdAmt1 * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if PayByDebitCdAmt1 > PendingShieldAmt then begin
                        ServiceCharges := ((PayByDebitCdAmt1 - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByDebitCdAmt1 - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - PayByDebitCdAmt1;
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format(PayByDebitCdAmt1 - ((PayByDebitCdAmt1 * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            j := 1;
            // 9) gift
            if ("GiftPay Method Code" <> '') and (GiftVoucherAmt <> 0) then begin
                PaymentMethod.Get("GiftPay Method Code");
                //MESSAGE('Gift');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if GiftVoucherAmt > PendingShieldAmt then begin
                        ArPayTypes[i, j] := Format(GiftVoucherAmt - PendingShieldAmt);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        PendingShieldAmt := PendingShieldAmt - GiftVoucherAmt;
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format(GiftVoucherAmt);
                j += 1;
                ArPayTypes[i, j] := '';
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := '0';
            ...
            j := 1;
            // 10) RTGS
            if ("RTGSPay Method Code" <> '') and ("RTGS Amount" <> 0) then begin
                PaymentMethod.Get("RTGSPay Method Code");
                //MESSAGE('RTGS');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "RTGS Amount" > PendingShieldAmt then begin
                        ArPayTypes[i, j] := Format("RTGS Amount" - PendingShieldAmt);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        PendingShieldAmt := PendingShieldAmt - "RTGS Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("RTGS Amount");
                j += 1;
                ArPayTypes[i, j] := '';
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := '0';
            ...
            j := 1;
            // 11) FIN
            if ("FinPay Method Code" <> '') and ("Finance Amount" <> 0) then begin
                PaymentMethod.Get("FinPay Method Code");
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Finance Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Finance Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Finance Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Finance Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Finance Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Finance Amount" - (("Finance Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            j := 1;
            // 12) MAPLE
            if ("Maple Buyback Method Code" <> '') and ("Maple Buyback Amt" <> 0) then begin
                PaymentMethod.Get("Maple Buyback Method Code");
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ArPayTypes[i, j] := PaymentMethod."Bal. Account No.";
                j += 1;
                ArPayTypes[i, j] := Format("Maple Buyback Amt");
                j += 1;
                ArPayTypes[i, j] := '';
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := '0';
            ...
            if ("Apple Buyback method  Code" <> '') and ("Apple Buyback Amt" <> 0) then begin
                ...
                ArPayTypes[i, j] := PaymentMethod."Bal. Account No.";
                j += 1;
                ArPayTypes[i, j] := Format("Apple Buyback Amt");
                j += 1;
                ArPayTypes[i, j] := '';
                j += 1;//Service Charge A/c;
                ArPayTypes[i, j] := '0';
          ...
          //MESSAGE('Finance');
            ...
            */
            ...
            // 14) PAYTM
            j := 1;
            if (paytmMethodCode <> '') and ("Paytm Transfer Amount" <> 0) then begin
                PaymentMethod.Get(paytmMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Paytm Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Paytm Transfer Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Paytm Transfer Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Paytm Transfer Amount" - ServiceCharges); //- PendingShieldAmt KPPL LM
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Paytm Transfer Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Paytm Transfer Amount" - (("Paytm Transfer Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            //15) EX
            j := 1;
            if (ExMethodCode <> '') and ("Amazon Transfer Amount" <> 0) then begin
                PaymentMethod.Get(ExMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Amazon Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Amazon Transfer Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Amazon Transfer Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Amazon Transfer Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Amazon Transfer Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Amazon Transfer Amount" - (("Amazon Transfer Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            //16) INSURANCE
            j := 1;
            if (InsuranceMethodCode <> '') and ("Insurance Transfer Amount" <> 0) then begin
                PaymentMethod.Get(InsuranceMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Insurance Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Insurance Transfer Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Insurance Transfer Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Insurance Transfer Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Insurance Transfer Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Insurance Transfer Amount" - (("Insurance Transfer Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            //17) CASHIFY
            j := 1;
            if (CashifyMethodCode <> '') and ("Cashify Transfer Amount" <> 0) then begin
                PaymentMethod.Get(CashifyMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Cashify Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Cashify Transfer Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Cashify Transfer Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Cashify Transfer Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Cashify Transfer Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Cashify Transfer Amount" - (("Cashify Transfer Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            j := 1;
            if (CashifyBonusMethodCode <> '') and ("Cashify Bonus Amount" <> 0) then begin
                PaymentMethod.Get(CashifyBonusMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Cashify Bonus Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Cashify Bonus Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Cashify Bonus Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Cashify Bonus Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Cashify Bonus Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Cashify Bonus Amount" - (("Cashify Bonus Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            //18) PINELAB
            j := 1;
            if (PinelabMethodCode <> '') and ("Pinelab Transfer Amount" <> 0) then begin
                PaymentMethod.Get(PinelabMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Pinelab Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    //IF PendingShieldAmt <> 0 THEN BEGIN
                    if "Pinelab Transfer Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Pinelab Transfer Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Pinelab Transfer Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Pinelab Transfer Amount";
                    end;
                    //END;
                end
                else
                    ArPayTypes[i, j] := Format("Pinelab Transfer Amount" - (("Pinelab Transfer Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
            ...
            j := 1;
            SalesOrderExtension.Reset;
            if SalesOrderExtension.Get("Document Type", "No.") then begin
            ...
            j := 1;
            if ("Loyalty Riwardz Method Code" <> '') and ("Loyalty Riwardz Tr.Amount" <> 0) then begin
                PaymentMethod.Get("Loyalty Riwardz Method Code");
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ArPayTypes[i, j] := Format(PaymentMethod."Bal. Account Type");
                ...
                ServiceCharges := ("Loyalty Riwardz Tr.Amount" * PaymentMethod."Service Charge Perc") / 100;
                if Shield and ("Total Shield Value" <> 0) and (PendingShieldAmt <> 0) then begin
                    if "Loyalty Riwardz Tr.Amount" > PendingShieldAmt then begin
                        ServiceCharges := (("Loyalty Riwardz Tr.Amount" - PendingShieldAmt) * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Loyalty Riwardz Tr.Amount" - PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                    else begin
                        ArPayTypes[i, j] := Format(0);
                        ServiceCharges := 0;
                        PendingShieldAmt := PendingShieldAmt - "Loyalty Riwardz Tr.Amount";
                    end;

                end
                else
                    ArPayTypes[i, j] := Format("Loyalty Riwardz Tr.Amount" - (("Loyalty Riwardz Tr.Amount" * PaymentMethod."Service Charge Perc") / 100));
                j += 1;
                ArPayTypes[i, j] := PaymentMethod."Service Charge Account No.";
                j += 1;//Service Charge A/c
                ArPayTypes[i, j] := Format(ServiceCharges);
    ...
    begin
        WITH SalesHeader DO BEGIN
            ...
            */
            ...
            "Posting No." := NoSeriesMgt.GetNextNo(PurchSetup."Invoice Nos.", "Posting Date", TRUE);
    ...
    begin
        with salesHeader do begin
            ...
            j := 1;
            PendingShieldAmt := "Total Shield Value";
            if ("CashPay Method Code" <> '') and (PayByCashAmt <> 0) and (PendingShieldAmt <> 0) then begin
                //MESSAGE('Cash');
                PaymentMethod.Get("CashPay Method Code");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
            ...
            j := 1;
            if ("ChqPay Method Code" <> '') and (PayByChqAmt <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(salesHeader."ChqPay Method Code");
                //MESSAGE('Chq');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
            ...
            j := 1;
            if ("CreditCdPay Method Code1" <> '') and (PayByCreditAmt1 <> 0) and (PendingShieldAmt <> 0) then begin
                //MESSAGE('Credit1');
                PaymentMethod.Get("CreditCdPay Method Code1");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if PayByCreditAmt1 > PendingShieldAmt then begin
                        ...
                        ServiceCharges := (PayByCreditAmt1 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt1 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt1;
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("CreditCdPay Method Code2" <> '') and (PayByCreditAmt2 <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("CreditCdPay Method Code2");
                //MESSAGE('Credit2');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if PayByCreditAmt2 > PendingShieldAmt then begin
                        ...
                        ServiceCharges := (PayByCreditAmt2 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt2 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt2;
                    end;
                    /// END;
                end;
            ...
            j := 1;
            if ("CreditCdPay Method Code3" <> '') and (PayByCreditAmt3 <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("CreditCdPay Method Code3");
                //MESSAGE('Credit3');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if PayByCreditAmt3 > PendingShieldAmt then begin
                        ...
                        ServiceCharges := (PayByCreditAmt3 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt3 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - (PayByCreditAmt3);
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("CreditCdPay Method Code4" <> '') and (PayByCreditAmt4 <> 0) and (PendingShieldAmt <> 0) then begin
                //MESSAGE('Credit4');
                PaymentMethod.Get("CreditCdPay Method Code4");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                ServiceCharges := 0;
                if PendingShieldAmt <> 0 then begin
                    if PayByCreditAmt4 > PendingShieldAmt then begin
                        ServiceCharges := (PendingShieldAmt * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PendingShieldAmt - ServiceCharges);
                        PendingShieldAmt := 0;
                    end
                        ...
                        ServiceCharges := (PayByCreditAmt4 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByCreditAmt4 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - PayByCreditAmt4;
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("DebitCdPay Method Code1" <> '') and (PayByDebitCdAmt1 <> 0) and (PendingShieldAmt <> 0) then begin
                ///MESSAGE('Debit');
                PaymentMethod.Get("DebitCdPay Method Code1");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if PayByDebitCdAmt1 > PendingShieldAmt then begin
                        ...
                        ServiceCharges := (PayByDebitCdAmt1 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByDebitCdAmt1 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - PayByDebitCdAmt1;
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("DebitCdPay Method Code2" <> '') and (PayByDebitAmt2 <> 0) and (PendingShieldAmt <> 0) then begin
                //MESSAGE('Credit4');
                PaymentMethod.Get("DebitCdPay Method Code2");
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if PayByDebitAmt2 > PendingShieldAmt then begin
                        ...
                        ServiceCharges := (PayByDebitAmt2 * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format(PayByDebitAmt2 - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - PayByDebitAmt2;
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("GiftPay Method Code" <> '') and (GiftVoucherAmt <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("GiftPay Method Code");
                //MESSAGE('Gift');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
            ...
            j := 1;
            if ("RTGSPay Method Code" <> '') and ("RTGS Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("RTGSPay Method Code");
                //MESSAGE('RTGS');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
            ...
            j := 1;
            if ("FinPay Method Code" <> '') and ("Finance Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("FinPay Method Code");
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Finance Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Finance Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Finance Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Finance Amount";
                    end;
                end;
                j += 1;
          ...
          //MESSAGE('Refund');
            ...
            //paytm
            j := 1;
            if (paytmMethodCode <> '') and ("Paytm Transfer Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(paytmMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Paytm Transfer Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Paytm Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Paytm Transfer Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Paytm Transfer Amount";
                    end;
                end;
                j += 1;
            ...
            //ex
            j := 1;
            if (ExMethodCode <> '') and ("Amazon Transfer Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(ExMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Amazon Transfer Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Amazon Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Amazon Transfer Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Amazon Transfer Amount";
                    end;
                end;
                j += 1;
            ...
            //INSURANCE
            j := 1;
            if (InsuranceMethodCode <> '') and ("Insurance Transfer Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(InsuranceMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Insurance Transfer Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Insurance Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Insurance Transfer Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Insurance Transfer Amount";
                    end;
                end;
                j += 1;
            ...
            //Cashify
            j := 1;
            if (CashifyMethodCode <> '') and ("Cashify Transfer Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(CashifyMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Cashify Transfer Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Cashify Transfer Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Cashify Transfer Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Cashify Transfer Amount";
                    end;
                end;
                j += 1;
            ...
            j := 1;
            if ("Loyalty Riwardz Method Code" <> '') and ("Loyalty Riwardz Tr.Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get("Loyalty Riwardz Method Code");
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
                ...
                if PendingShieldAmt <> 0 then begin
                    if "Loyalty Riwardz Tr.Amount" > PendingShieldAmt then begin
                        ...
                        ServiceCharges := ("Loyalty Riwardz Tr.Amount" * PaymentMethod."Service Charge Perc") / 100;
                        ArPayTypes[i, j] := Format("Loyalty Riwardz Tr.Amount" - ServiceCharges);
                        PendingShieldAmt := PendingShieldAmt - "Loyalty Riwardz Tr.Amount";
                    end;
                end;
                j += 1;
            ...
            //Pinelab
            j := 1;
            if (PinelabMethodCode <> '') and ("Pinelab Transfer Amount" <> 0) and (PendingShieldAmt <> 0) then begin
                PaymentMethod.Get(PinelabMethodCode);
                //MESSAGE('Finance');
                ArPayTypes[i, j] := PaymentMethod.Code;
                j += 1;
    ...
    begin
        ...
        or (SalesInvoiceHeader."E-Invoice Cancel Remark" <> xSalesInvoiceHeader."E-Invoice Cancel Remark");
    end;

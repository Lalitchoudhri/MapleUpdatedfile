tableextension 50063 "PurchaseLine" extends "Purchase Line"
{
    ...
    begin
        WITH SalesHeader DO BEGIN
            ...
            SETFILTER("No.", '%1', SalesOrderNo);
            IF FINDFIRST THEN BEGIN
                CALCFIELDS("Maple Buyback Amt");
                //VALIDATE(TotalPayByCustomer);
                TotalPayByCustomer := PayByCashAmt + PayByCreditAmt1 + PayByCreditAmt2 + PayByCreditAmt3 + PayByCreditAmt4 + PayByDebitCdAmt1 + PayByChqAmt + GiftVoucherAmt + "RTGS Amount" + "Finance Amount" + "Maple Buyback Amt" + "Apple Buyback Amt" + "Apple Refund Amt";
                TotalPayByCustomer := TotalPayByCustomer + PayByDebitAmt2 + LineAmount - xLineAmount;
                MODIFY;
                COMMIT;
                CALCFIELDS("Maple Buyback Amt");
            END;

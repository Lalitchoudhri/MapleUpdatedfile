    ...
    fields
    {
                ...
                IF (Rec.Type = Rec.Type::Item) AND (Rec."Document Type" = Rec."Document Type"::Order) then BEGIN
                    ...
                    if Rec."Item Serial No." <> '' then begin
                        ...
                        if ile2.FindFirst() then begin
                            if ILE2."Serial No." <> Rec."Item Serial No." then begin
        ...
        field(50021; "Shield Type"; Option)
        {
            ...
            begin
                ...
                //WIN270+++
                IF "Shield Type" = "Shield Type"::Shield THEN BEGIN
                ...
                END;
                IF (Type = Type::"G/L Account") AND ("No." = '310014') THEN BEGIN
        ...
        field(50049; "Discount Type"; Code[20])
        {
            ...
            var
                ...
                Discounting1: Record Discounting;
                DiscountingRec: Record Discounting;
                DiscountingRec1: record Discounting;
                Item: Record Item;
        ...
        field(50056; "Entry No"; Integer)
        {
            ...
            var
                ...
                DiscountingF: Record Discounting;
                Discounting1: Record Discounting;
                DiscountingRec: Record Discounting;
                Item: Record Item;
                ReCSH: Record "Sales Header";
            ...
            begin
                ...
                IF DiscountingB.FindFirst() THEN BEGIN
                ...
                IF (Rec."Entry No" <> '8') Or (Rec."Discount Type" <> 'NO DISCOUNT') THEN begin
                    IF Item.Get(Rec."No.") THEN BEGIN
                        DiscountingRec.Reset();
        ...
        modify("No.")
        {
            ...
            begin
                ...
                If Item.get(Rec."No.") then begin
                    ...
                    IF ((SalesHeader."Sales Order Type" = SalesHeader."Sales Order Type"::Retail) OR (SalesHeader."Sales Order Type" = SalesHeader."Sales Order Type"::B2B)) AND (Type = Type::Item) THEN BEGIN//AND (SalesHeader.Structure <> '')
